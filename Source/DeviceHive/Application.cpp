...
//-----------------------------------------------------------------------------
{
   ...
   //strcpy(cServerUrl, "http://nn7552.pg.devicehive.com/api");
   //strcpy(cServerUrl, "http://54.148.189.110/api");
   strcpy(cServerUrl, "http://91.247.221.214:8888/api/");
   //strcpy(cServerUrl, "http://91.247.221.214/brokenapi");
   //strcpy(cServerUrl, "http://ecloud.dataart.com/ecapi9");
}
...
//-----------------------------------------------------------------------------
{
   // TODO: Implement Instrument-specific command handling here.
...
//-----------------------------------------------------------------------------
{
   if (result == DH_NO_ERROR)
   {
      LOG(Tools::Logger::lmInfo, "The Instrument has been successfully registered!");

      cJSON* jLastNotifTimeStamp = cJSON_GetObjectItem(jRegistrationInfo, "lastNotification");
      if (jLastNotifTimeStamp)
      {
         jLastNotifTimeStamp->valuestring;// This value should be important for you
         LOG1(Tools::Logger::lmInfo, "Got LastNotif TimeStamp (%s)!", jLastNotifTimeStamp->valuestring);
      }
   ...
   {
      bool isInterrupted = result->Interrupted();
      LOG(Tools::Logger::lmInfo, "Failed to register the instrument.");
      HandleError(result);
...
//-----------------------------------------------------------------------------
void CApplication::BatchNotificationsExample()
//-----------------------------------------------------------------------------
{
   cJSON* jBatchNotifications = cJSON_CreateObject();
   cJSON* jNotifications = cJSON_CreateArray();
   char TimeStamp[32 + 1];
   char Name[32 + 1];
   const size_t cBundleSize = 5;

   for(size_t i = 0; i < cBundleSize; ++i)
   {

      cJSON* jNotification = cJSON_CreateObject();
      cJSON* jParameters = cJSON_CreateObject();

      sprintf(Name, "Notification %d of %d", i + 1, cBundleSize);
      cJSON_AddItemToObject(jNotification, "notification", cJSON_CreateString(Name));

      cJSON_AddItemToObject(jParameters, "number", cJSON_CreateNumber(i + 1));
      cJSON_AddItemToObject(jNotification, "parameters", jParameters);

      m_pGateway->GetTimeStamp(TimeStamp);
      cJSON_AddItemToObject(jNotification, "localTimestamp", cJSON_CreateString(TimeStamp));

      cJSON_AddItemToArray(jNotifications, jNotification);
   }

   cJSON_AddItemToObject(jBatchNotifications, "notifications", jNotifications);
   RetryableObject* pBatchNotifications = new RetryableObject(jBatchNotifications);

   LOG1(Tools::Logger::lmInfo, "Sending BatchNotifications{0x%X}", pBatchNotifications);

   m_pGateway->AsyncSendNotification(pBatchNotifications,
      (CGateway::GatewayListener::instrOnNotifyCB)&CApplication::OnNotificationSent);
}
...
//-----------------------------------------------------------------------------
{
   do
   {
      ...
      if (GetPushButton())
      {
         ...
         if (1)
         {
            m_pApp->BatchNotificationsExample();
         }
      }
   }while(!m_Terminated);
