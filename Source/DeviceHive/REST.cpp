...
//-----------------------------------------------------------------------------
{
   ...
   LOG1(Tools::Logger::lmInfo, "Registering the instrument, Task{0x%X}", pHttpTask);

   pHttpTask->AddPath("device");
   pHttpTask->AddPath(m_pGateway->GetInstrumentID());
   pHttpTask->FinishUrl();
   pHttpTask->AddHeader(HTTP::Content_Type, "application/json");
   pHttpTask->AddHeader(HTTP::Accept, "application/json");
   pHttpTask->AddHeader(HTTP::Connection, "keep-alive");

   pHttpTask->AddHeader(HTTP::AuthDeviceID, m_pGateway->GetInstrumentID());
   pHttpTask->AddHeader(HTTP::AuthDeviceMac, m_pGateway->GetInstrumentMAC());
...
//-----------------------------------------------------------------------------
{
   ...
   pHttpTask->AddHeader(HTTP::Connection, "keep-alive");
   pHttpTask->AddHeader(HTTP::AuthDeviceID, m_pGateway->GetInstrumentID());
   pHttpTask->AddHeader(HTTP::AuthDeviceKey, m_AuthKey);
   pHttpTask->Finalize();
...
//-----------------------------------------------------------------------------
{
   ...
   if (result == DH_NO_ERROR)
   {
      ...
      else
      {
         ...
         result = FormatError(task->GetHttpStatus(), ErrorDH::ET_HttpStatus,
                              ErrorDH::EP_RestOnNoify,
                              "Failed to send a Notidf: Invalid HTTP status");
      }
   }
   else
   {
