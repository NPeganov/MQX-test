...
//-----------------------------------------------------------------------------
{
   // Here is an example of the instrument meta data. For now it is just dummy data,
   // except of MAC address and Serial number, which are taken from the Application layer

   LOG(Tools::Logger::lmInfo, "Registering the instrument");
...
*/
   ...
   cJSON_AddItemToObject(jEquipment1,"type",cJSON_CreateString("Buchi Equipment type1"));
   ...
   cJSON_AddItemToObject(jEquipment2,"type",cJSON_CreateString("Buchi Equipment type2"));
   ...
   */
   ...
   cJSON_AddItemToObject(jDeviceClass,"name",cJSON_CreateString("Buchi device class"));
   ...
   cJSON_AddItemToObject(jDeviceNetwork,"name",cJSON_CreateString("BuchiNetwork"));
   cJSON_AddItemToObject(jDeviceNetwork,"description",cJSON_CreateString("Buchi Instrument Network"));

   cJSON_AddItemToObject(jDeviceData,"SomeDataIsHere",cJSON_CreateString("SomeDeviceData"));
   cJSON_AddItemToObject(jDevice,"id", cJSON_CreateString(m_Instrument.m_Serial));                  // <=== Serial
   cJSON_AddItemToObject(jDevice,"key", cJSON_CreateString(m_Instrument.m_MacAdressStr));           // <=== MAC
   cJSON_AddItemToObject(jDevice,"name",cJSON_CreateString("MQX BUCHI INSTRUMENT"));
...
//-----------------------------------------------------------------------------
{
   ...
   _mem_zero(m_Instrument.m_MacAdressStr, cStrMacLen);
   _mem_zero(m_Instrument.m_Serial, cSerialLen);

   if (m_pApp->GetMacAddress(macAdress))
   {
      sprintf(m_Instrument.m_MacAdressStr, "%02X%02X%02X%02X%02X%02X",
              macAdress[0],
              macAdress[1],
              macAdress[2],
              macAdress[3],
              macAdress[4],
              macAdress[5]);
      LOG1(Tools::Logger::lmTrace, "The Gateway got MAC: %s", m_Instrument.m_MacAdressStr);

      result = SetupNetwork(macAdress, Enable);
      if (result != DH_NO_ERROR)
        return (UInt32)result;

      if (!m_pApp->GetSerial(m_Instrument.m_Serial, cSerialLen))
      {
...
//-----------------------------------------------------------------------------
const char* CGateway::GetInstrumentID()
//-----------------------------------------------------------------------------
{
   return m_Instrument.m_Serial;
}

//-----------------------------------------------------------------------------
const char* CGateway::GetInstrumentMAC()
//-----------------------------------------------------------------------------
{
   return m_Instrument.m_MacAdressStr;
}
...
//-----------------------------------------------------------------------------
{
   ...
   if (result == DH_NO_ERROR)
   {
      LOG(Tools::Logger::lmInfo, "Got Instrument registered!");
      success = true;
   }

   if(m_InRegisterCallback)
   {
      LOG2(Tools::Logger::lmInfo, "Gateway{0x%X} has Registered instrument and calling callback {0x%X}",
